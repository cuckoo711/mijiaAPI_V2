...
- 凭据加密密钥文件 `.credential_key` 读取时不再对二进制内容 `strip()`，避免首尾恰为空白字节时偶发 `MAC check failed`（表现为 CI / 本地单测间歇失败）。

### 文档

- 同步开发环境、API Server、配置与故障排查：仓库结构、`deploy/` 路径、凭据加密说明与 MAC 失败处理。

## v3.7.2 - 2026-07-21

### 整理

- 运维脚本从根目录 `scripts/` 迁入 `deploy/scripts/`（clean / release notes / 设备规格）。
- 修正脚本相对仓库根的路径解析；同步文档与 CI 引用。
...
- SDK / Server 默认数据路径统一为 `configs/`（凭据 `configs/credential.json`，缓存 `configs/cache`）。
- 旧版项目根 `.mijia/` 迁移到 `configs/` 后**直接删除**旧目录与 `.mijia_backup*`，不再保留备份。
...
- v2→v3 数据迁移改为幂等：已迁移后的 `.mijia` 残留会被清理且不再每次启动打印「迁移完成」。
- 清理无用的 `configs/server/cache` 与空的 `.mijia_backup`，避免双份磁盘缓存。

### 运维

- 新增 CLI `status`（版本/路径/库与缓存体积）与 `purge-cache`（清理过期或全部磁盘缓存）。
- `deploy/mijia-server.service` 补充可选 `MemoryMax` 注释（空闲约 40–80MB RSS）。

## v3.6.6 - 2026-07-21

### 修复

- 管理台同步轮询绑定后端返回的 `task_id`，忽略上一轮残留的终态进度。

### 文档

- `deploy/mijia-server.service` 补充 Cookie/CSRF 同源假设说明。

## v3.6.5 - 2026-07-21

### 修复

- 同步开始时立即写入新的 `running` progress（含 `task_id`），避免轮询短暂读到上一轮 `completed`。
- 同步锁在终态后立刻释放，再安排延迟清理，便于连续发起下一轮同步。
...
- 同步进度延迟清理改为可配置延迟，并稳定相关单测，避免整仓跑测时偶发清空新一轮 progress。

### 改进

- 凭据「即将过期」判定与 `credential_refresh_before_seconds` 对齐（不再写死 48 小时）。
- 管理台 Element Plus 改为按需引入，CSS 体积显著下降。
